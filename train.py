...
# x = Dropout(.4)(x)
# x = Flatten()(x)
# headModel = Dense(	len(config.CLASSES),
...
plot_training(H, config.EPOCH_HEAD, config.WARMUP_PLOT_PATH)

model.save(config.MODEL_PATH_HEAD)
# model.save_weights(config.MODEL_PATH_HEAD)
...
print(classification_report(testGen.classes, predIdxs,
	target_names=testGen.class_indices.keys()))
plot_training(H, config.EPOCH_ALL, config.UNFROZEN_PLOT_PATH)
